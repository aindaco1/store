...
- Added an idempotent, path-scoped Cloudflare Cache Response Rule for admin HTML that sets `no-transform` and private/no-store directives, preventing Cloudflare JavaScript Detection and automatic Web Analytics injection from conflicting with the strict admin CSP or browser privacy blockers.
