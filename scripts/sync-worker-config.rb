...
def build_mirror_values(config, existing)
...

  {
    ...
    'EVENT_FOLLOWUP_MISSION_ES' => event_followup.key?('mission_es') ? event_followup['mission_es'].to_s : existing['EVENT_FOLLOWUP_MISSION_ES'],
    ...
    'CHECKOUT_HOLD_SECONDS' => [[checkout.fetch('hold_seconds', 600).to_i, 600].max, 1800].min.to_s,
    ...
    'WORKERS_CACHE_ENABLED' => cache.key?('workers_enabled') ? (cache['workers_enabled'] ? 'true' : 'false') : existing['WORKERS_CACHE_ENABLED'],
    'WORKERS_CACHE_TELEMETRY_ENABLED' => cache.key?('workers_telemetry_enabled') ? (cache['workers_telemetry_enabled'] ? 'true' : 'false') : existing['WORKERS_CACHE_TELEMETRY_ENABLED'],
    ...
    'WORKERS_CACHE_ADMIN_ANALYTICS_ENABLED' => cache.key?('workers_admin_analytics_enabled') ? (cache['workers_admin_analytics_enabled'] ? 'true' : 'false') : existing['WORKERS_CACHE_ADMIN_ANALYTICS_ENABLED'],
    'WORKERS_CACHE_ADMIN_INVENTORY_ENABLED' => cache.key?('workers_admin_inventory_enabled') ? (cache['workers_admin_inventory_enabled'] ? 'true' : 'false') : existing['WORKERS_CACHE_ADMIN_INVENTORY_ENABLED'],
    'WORKERS_CACHE_ADMIN_DOWNLOADS_ENABLED' => cache.key?('workers_admin_downloads_enabled') ? (cache['workers_admin_downloads_enabled'] ? 'true' : 'false') : existing['WORKERS_CACHE_ADMIN_DOWNLOADS_ENABLED'],
