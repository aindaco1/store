...
# Avoid unnecessary Wrangler reloads, and never expose a partially written JS
# module to its watcher when the local repository service regenerates it.
unless File.exist?(OUTPUT_PATH) && File.read(OUTPUT_PATH) == output
  Tempfile.create(['.catalog-snapshot.', '.tmp'], OUTPUT_DIR) do |file|
    file.write(output)
    file.flush
