...
- **Recovery data control:** The separate-office iMac is acceptable only as an encrypted, checksum-verified, append-only second-location copy with local decrypt verification; it must not be exposed as a public file server. Restricted live Stripe comparison and first fully protected recurring drill remain credential/operator gates.
