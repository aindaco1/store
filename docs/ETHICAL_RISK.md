...
- **Unwanted or repeated messaging:** A durable queue could resend stale email or ignore consent. Mitigation: payloads and idempotency are frozen, processing uses leases/backoff, ambiguity stops for review, abandoned-cart reminders require consent/unsubscribe, and permanent bounce/complaint suppression is shared. Post-event commercial email is limited to one deduplicated confirmed purchaser per event, requires repository opt-in before the scheduled cutoff, records activation time, automatically reconciles earlier confirmed purchasers only when due, fails closed for legacy activation state, carries a durable optional-email opt-out, and never suppresses essential receipts, tickets, security, or order updates. The Settings preview renders copy without scanning or exposing customer data, and no general-purpose historical backfill control is available. Short-lived one-time links remain immediate. Residual owner: Store messaging operator, reviewed 2026-08-26.
...
## v1.0.9 Policy, Crawl, and Admin Review

- **Final-sale pressure and consumer harm:** A no-returns default can be misleading or used to deny remedies for Store error, damaged/defective/incorrect/missing goods, delayed/canceled fulfillment, duplicate charges, fraud, or rights supplied by law. Mitigation: stable localized policy links and the pre-payment checkout notice disclose final-sale status; Terms preserve those separate remedies, make the seven-day damage-report request a verification guideline rather than a waiver, and require delay consent or cancellation/refund when physical merchandise cannot ship on time. Residual owner: Store operator/legal reviewer, reviewed 2026-07-14.
- **Policy/metadata drift:** Customer copy, checkout disclosure, JSON-LD, and Worker settings could describe different return programs. Mitigation: one configured return category drives conditional Terms and structured data; synchronization clears finite-window fields when returns are not permitted; generated SEO tests reject a fictitious return window.
- **Crawl visibility and accidental disclosure:** Text/XML drift or a permissive selector could submit private/admin/order URLs, while generated build dates could misrepresent freshness. Mitigation: one selector feeds both formats, explicit content dates are the only `lastmod` source, and generated plus deployed audits reject private URLs, duplicates, invalid/future dates, parity drift, malformed responses, and blocking interstitials.
- **Admin power, privacy, and low-connectivity access:** Session revocation and audit exports expose sensitive operational controls, and a growing core bundle delays all admin tasks. Mitigation: canonical authorization/CSRF/redaction rules stay server-owned; the localized review UI is lazy and receives only a narrow dashboard API; executable per-bundle and aggregate budgets prevent the extraction from becoming an unbounded second dashboard.

## v1.2.0 RSVP Registration Review

- **Attendee privacy and surveillance:** Custom questions can collect accessibility, dietary, age-group, or other sensitive information about people who did not operate checkout. Mitigation: registration is opt-in per RSVP product, question count/length and party size are bounded, authored definitions stay repository-backed, answers stay out of browser storage, public catalog data, Stripe metadata, logs, audit records, and email content, and the operator guidance says to ask only what event operations require. Authoritative orders and the derived admin model remain restricted/private-no-store and follow encrypted backup retention. Residual owner: Store event operator, reviewed 2026-08-18.
- **Unauthorized disclosure and export misuse:** Named attendee search, response review, and CSV export can expose a guest list. Mitigation: existing Store admin session, `store:read`, CSRF/origin mutation, private/no-store, and encrypted-export handling remain in force; response values are escaped in browser/email rendering and are never interpolated as HTML. Operators must remove working CSV copies when no longer needed. Residual owner: Store event operator, reviewed 2026-08-18.
- **Eligibility and access errors:** Stale pages or tampered answers could admit excess guests, submit after a deadline, or invent a choice. Mitigation: the browser is advisory; the Worker resolves the current canonical product, validates open/close timestamps, exact attendee count, party limit, required fields, allowed options, and inventory before confirming the order. Historical orders retain their canonical question snapshot when product definitions later change.
- **Compatibility and unequal access:** A complex RSVP form could break checkout for merch, downloads, paid tickets, legacy RSVPs, keyboard users, Spanish-language users, or low-connectivity visitors. Mitigation: `event_details.registration` is an explicit RSVP-only gate; other products use the unchanged path; the UI uses labels, fieldsets, legends, focusable error targets, localized system copy, no new network dependency, and in-memory drafts. Regression, localization, accessibility, generated-site, and performance gates remain release requirements.
- **Payment confusion and unnecessary provider exposure:** Showing payment controls on a free registration could imply a charge or load a payment provider that is not needed. Mitigation: zero-total checkout omits tip and payment-method controls, uses a direct **Complete order** action, and does not prewarm Stripe; paid and mixed totals retain the existing PaymentIntent path, and a Worker-returned PaymentIntent overrides a stale browser estimate.
- **Admin power and attendance manipulation:** Per-attendee check-in changes event attendance records. Mitigation: mutations retain the existing `fulfillment:manage`, Store scope, session, CSRF, order-ready, and audit boundaries; unknown attendee IDs fail closed; partial state rolls up deterministically; and legacy item-level records remain reversible. No public check-in mutation or payment/provider action is added.
- **Future-feature overclaiming:** Waitlists, self-service changes, private invitations, event broadcasts, offline scanning, walk-ins, series, and RSVP analytics introduce new identity, capacity, communication, device, and inference risks. Mitigation: these remain explicitly unshipped roadmap items with server-authoritative capacity, purpose-limited messaging, narrow roles, privacy, recovery, localization, accessibility, and ethical-review gates required before implementation or public claims.

## Admin Invitation Review

- **Misleading access control (2026-09-09):** The Store checkbox offered an invalid choice because every limited Store admin requires Store access. The user editor now assigns the existing Store scope automatically and shows Name, Email, and Role. Worker validation and super-admin restrictions remain authoritative. Browser coverage verifies new users, role changes, and self-account protection; the [rollout record](release-evidence/2026-09-09-admin-access-isolation.md) records live verification.

- **Cross-application access and user-list corruption (2026-09-09):** Store and Pool shared a KV namespace and unprefixed auth keys. Store now uses separate keys for users, legacy per-user records, login nonces, sessions, minimized login history, and audit events. No unscoped fallback or automatic role conversion is permitted. The migration requires explicit Store membership and preserves Pool campaign assignments; existing Store sessions require fresh sign-in. Limited admins retain Store operations while settings and user management are denied at the API. Local shared-namespace tests cover isolation, token rejection, role boundaries, and readable validation failures. Deployment and live membership verification remain separate evidence.

- **Forwarded or intercepted admin invitations:** A direct access email is a bearer credential until it is redeemed or expires. Mitigation: new-admin notices reuse the existing signed login exchange, expire after 15 minutes, are deleted on first successful exchange, and re-check the current KV-backed admin-user list before issuing a session. Removing the user invalidates an unredeemed invitation, expired links fall back to the normal sign-in flow, and no password or reusable secret is introduced. Residual owner: Store operator, reviewed 2026-08-27.

## Product Media Publishing Review — 2026-09-06
