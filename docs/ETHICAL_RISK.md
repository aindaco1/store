...
- **Backup misuse:** Captured order/admin/R2 data could expose customers or copyrighted files. Mitigation: sensitive capture requires exact acknowledgement, public-key encryption plus decryptability verification, protected approval, private temporary files, preview-only restore, and encrypted/sanitized artifacts. No unattended production restore path exists.
- **Automation and economic harm:** A restore during active checkout or webhook settlement could duplicate or lose payment/inventory effects. Mitigation: the quarterly job uses Worker-wide request/error preflight, production concurrency, fresh one-time admin access, preview resources, idempotency fixture coverage, and no Stripe/email/check-in command path. Production restore retains separate maintenance and second-review gates.
...
- **Residual owner evidence:** Store operations must approve RPO/RTO/retention, review the dedicated recovery identity and long-term off-account destination, supply a fresh one-time token for each protected drill, verify decryption on a second isolated device/location, and record the first successful live preview drill before the recovery objective is described as proven.
