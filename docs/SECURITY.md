...
| Admin user invitation | New-admin access email | Generated only after a super-admin and CSRF-protected user save; reuses the standard 15-minute one-time login exchange and re-checks current admin authorization before issuing a session. |
...
- A path-scoped Cloudflare Cache Response Rule for `/admin/` and `/es/admin/` sets `private, no-store, no-transform, max-age=0, must-revalidate`, preventing Cloudflare JavaScript Detection, Rocket Loader, and automatic Web Analytics injection without allowing inline scripts. The sandboxed post-event email preview shell lives at `/admin/email-preview-frame/` so it inherits that same prefix rule; injecting analytics into its intentionally opaque origin would otherwise produce cross-origin and RUM CORS errors. Deploy and provider evidence verify the dashboard headers and injection markers without privileged credentials, and post-deploy acceptance must confirm the same policy on the preview shell. The optional API reconciliation command requires a separate Cache Rules Edit token and never replaces unrelated zone rules.
