...
Scheduled operational evidence includes **Workers Cache Evidence** nightly, **Recovery Readiness** weekly, **Production Posture** weekly, **Localization Review** monthly, and the protected quarterly recovery preflight. Production Posture synchronizes the repository's production-shaped Worker configuration inside its disposable checkout, prefers canonical production origins for provider probes, and treats required secret names, bindings, origins, and runtime configuration as its pass/fail configuration gate. Optional Stripe, Resend, and USPS API checks without workflow credentials are recorded as `manual` / not run rather than inferred failures or inferred passes; an attempted provider check that returns an explicit failure remains actionable. The workflow opens or updates a sanitized issue on actionable drift and never mutates deployed runtime configuration or provider state. Localization Review produces a human-review packet without asserting translation approval.
...
1. Admin edits a product, applies a bulk status, or arranges product rows in
   **Products**.
2. Status changes remain pending until the existing visible action is used:
   **Activate product**, **Save as draft**, **Archive product**, or **Mark sold
   out**. Reordering remains local until **Save order** is used.
3. Browser submits the matching existing endpoint:
...
8. Only a successful matching run triggers the product-list refresh. At that
   ...
   but unavailable to purchase. Product order saves keep the arranged rows on
   screen until this refresh. Failure leaves the saved repository change and
   prior deployed list visibly distinct and provides the GitHub run link when
   known.
