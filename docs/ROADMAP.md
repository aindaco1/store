...
- [x] Cloudflare Workers Cache performance and read-efficiency hardening
  ...
  - Real Cloudflare 30-sample edge latency/hit-ratio evidence, candidate enablement, immediate post-invalidation KV-propagation deduplication, and complementary Lighthouse budgets remain future operations work.
...
- [ ] Production quality gates and admin operations hardening
  ...
  - Prevent an immediate no-change Orders request from repeating a bounded index rebuild while a newly written KV index propagates. Reuse the existing order-index/cache policy and mutation invalidation contracts; do not add per-order counters, customer data to cache keys, or a parallel projection unless production sample evidence justifies it.
...
- [ ] Cloudflare Workers Cache production evidence and rollout
  - Run the completed benchmark/comparison tooling against authorized cache-disabled and cache-enabled Cloudflare deployments. Use at least 30 samples for repeated warm/no-change/search cases, keep cold/post-purge checks bounded because every purge creates an audit KV write, and record p50/p95/p99 latency, response bytes, `Cf-Cache-Status`, Worker CPU where available, and emitted operation budgets. The bounded production Orders probe has already shown a 3-4 ms repeat `HIT` with zero order-data KV reads/lists, but it is not the required 30-sample comparison.
  - Continue collecting deployment-scoped Cloudflare observability until normal traffic provides a meaningful aggregate `HIT`/`MISS`/`UPDATING`/`BYPASS` ratio. Podman and in-process tests prove application behavior but must not be represented as proof of Cloudflare edge caching.
  - Require 30-sample warm no-change Orders and Analytics hits to perform zero order-data KV list/get work after gateway authentication, improve p95 latency by at least 40 percent over uncached fallback, remain fresh after every covered mutation/purge, and save enough CPU/backend reads to justify the additional billed inner Workers request.
...
- [ ] Backup, restore, and disaster recovery operations
  ...
  - Use the completed retention planner, then verify at least one durable off-device copy outside the production Cloudflare account. The protected workflow now requires and verifies S3 upload, but no off-account S3 destination or credentials are configured; a 90-day encrypted GitHub artifact is not the approved archive.
