...
- [x] Cloudflare Workers Cache performance and read-efficiency hardening
  ...
  - `.github/workflows/workers-cache-evidence.yml` runs at `03:17 America/Denver`, queries aggregate hit ratios, checks a recent cache-read ceiling before its scoped two-read probe, and stores sanitized evidence only. The dedicated evidence credential can read only the fixed cache probe surface; it cannot return Store rows, purge, mutate settings, or invoke payment/fulfillment operations.
...
- [ ] Backup, restore, and disaster recovery operations
  ...
  - Rehearse a full live captured snapshot in an isolated local/preview target. The representative synthetic coverage is complete, but it must not be represented as proof that a captured production snapshot and every external provider class are recoverable.
  ...
  - Use the completed retention planner, then verify at least one durable off-device copy outside the production Cloudflare account. A 90-day encrypted GitHub drill artifact is not a substitute for the approved 7-daily/5-weekly/12-monthly archive destination.
  - Enable and run the protected quarterly workflow only after its dedicated age recovery identity, fresh one-time super-admin token, preview namespaces/bucket, required reviewer, and off-account retention destination are reviewed. Record duration against RTO, verified classes, skips, failures, owner, and corrective actions before calling the recurring drill objective proven.
