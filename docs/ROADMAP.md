...
- [x] Cloudflare Workers Cache performance and read-efficiency hardening
  ...
  - The materialized v2 order index now uses a seven-day safety TTL because every order-changing path explicitly invalidates it. This removes the measured ten-minute 417-order rescan cliff while retaining bounded recovery from a missed invalidation.
