...
- [ ] `v1.0.8` production quality and admin operations closure
  - Verify the expected live Stripe webhook endpoint read-only. The local Stripe key is test-mode and belongs to Pool, so Store does not mutate that non-production endpoint merely for readiness evidence.
...
- [ ] `v1.0.8` Cloudflare Workers Cache evidence and rollout decision
  - Wait for the configured four-hour deployment-stability window and collect deployment-scoped sanitized Cloudflare evidence; Podman/in-process results are not edge-cache proof.
