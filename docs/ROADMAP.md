...
- `v1.0.6` added Workers Cache performance/read-efficiency hardening, versioned backup/restore/disaster-recovery automation, and Podman-backed default runtime tests. `v1.0.7` added production quality budgets, fixed-key order-index rebuild deduplication, admin session/audit/download-abuse hardening, scheduled posture/localization review, and approved recovery objectives/retention controls. The `v1.0.8` release candidate adds product/media/add-on authoring, payment-integrity/reconciliation hardening, and durable Store email delivery.
- Published tags are immutable. The published `v1.0.7` tag remains unchanged; the Storefront, Worker, canonical configuration, release notes, and evidence are packaged as the `v1.0.8` candidate.
- `v1.0.8` code-path evidence is complete. Production deploy/provider validation, an evidence-based Workers Cache rollout decision, and the protected captured-data recovery drill remain explicit release-closure gates rather than inferred passes.
...
- [x] Production quality gates and admin operations hardening
  ...
  - English/Spanish completeness and rendered QA cover download creation, product editing, order lookup, checkout/coupon failures, reminders, admin status, email, fulfillment, and the new `v1.0.8` media controls. Store-owned media labels/errors use the shared runtime-message catalog, while user-authored product/media data remains unchanged; active deployed-language sampling remains a release operations gate.
...
- [x] Release evidence automation
  ...
  - The `v1.0.8` adaptation and release-closure record separates local code-path passes from deployment, provider, edge-cache, and protected recovery evidence; unavailable production authority remains an explicit open gate.
...
- [ ] `v1.0.8` production quality and admin operations closure
  - Review and commit the exact green working tree, then rerun `npm run test:premerge`, the source-hashed localization packet, rendered i18n/SEO evidence, production posture, and the release-smoke wrapper from that immutable candidate; attach all explicit passes, warnings, and credential-dependent skips to [release-evidence/v1.0.8.md](release-evidence/v1.0.8.md).
  - Rerun production posture with the new dedicated fulfillment secret. Verify the expected live Stripe webhook endpoint read-only, correct the test endpoint's required PaymentIntent subscriptions or record its explicit non-production disposition, and use the existing GitHub provider workflow for DNS API evidence.
  - Deploy only the reviewed commit. Sample active English/Spanish routes for canonical/alternate links, Open Graph/Twitter metadata, Product/Breadcrumb/Organization JSON-LD, merchant return policy, admin status copy, and the new media controls.
  - Configure and verify the signed Resend delivery webhook, record one provider test event without customer data, inspect the first durable outbox and read-only reconciliation cycles, and sample unauthenticated/private/no-store failure boundaries before closure.
  - Keep translated product-body metadata QA deferred until real creator-approved translated product copy exists. Existing fallback/`availableLanguage` behavior remains tested and this content dependency does not justify fabricated translations.
  - Update only the owning performance, security, accessibility, i18n, Podman, SEO, dashboard, testing, and workflow docs when the closure work changes their contract.

- [ ] `v1.0.8` Cloudflare Workers Cache evidence and rollout decision
  - After deploying the reviewed `v1.0.8` candidate, wait for the configured four-hour deployment-stability window and collect deployment-scoped sanitized Cloudflare evidence. Podman/in-process results must not be represented as edge-cache proof.
  - Leave Orders disabled because its completed 30-sample comparison failed the 40-percent p95 benefit gate. Evaluate Analytics next with at least 30 disabled/enabled samples, zero warm/no-change order-data KV list/get work, mutation/purge freshness, and enough CPU/backend-read savings to justify the billed inner Worker request.
  - Enable Analytics only if every gate passes, monitor hit ratio and stale/fallback behavior, and record rollback/kill-switch evidence. If it fails, keep it disabled and record the evidence-based no-rollout decision; a justified no-rollout counts as release completion.
  - Evaluate order-derived inventory and download readiness sequentially only after the prior candidate closes. Keep `cross_version_cache: false` unless rollback/version-tag purge tests and measured benefit justify a later change.
  - Attach sanitized benchmark and Cloudflare dashboard summaries without tokens, cookies, response bodies, customer data, or PII-bearing cache keys.

- [ ] `v1.0.8` backup, restore, and disaster recovery operational proof
  - Capture a fresh encrypted `v1.0.8` snapshot using a fresh one-time super-admin token so admin CSV/readiness/download exports are included. Record duration and aggregate Cloudflare operations, then compare them with the approved four-hour active-sales RPO and documented RTO.
  - Copy or download the encrypted archive to a second isolated device/location and prove checksum plus decryption there. Keep plaintext temporary and remove it on success or failure.
  - Use the provisioned restricted live-mode Payment Intents read-only Stripe key for the aggregate captured-order comparison. Reject mode mismatch and perform no Stripe mutation.
  - Enable the protected quarterly workflow only after the off-account target, fresh admin token, restricted Stripe key, age identity, preview KV/R2 targets, required reviewer, and approved RPO/RTO/retention inputs are present. Run the preview-only restore, readback, exact-snapshot cleanup, and best-effort failure cleanup.
  - Record verified classes, quarantined/derived skips, durations, operation counts, live Stripe comparison, off-account receipt, second-device decryption, zero-residual preview cleanup, owner, and corrective actions in [release-evidence/v1.0.8.md](release-evidence/v1.0.8.md). Production restore remains a separate manual incident procedure.
