...
- [ ] Cloudflare Workers Cache production evidence and rollout
  ...
  - Reconsider Orders only after a material traffic, data-shape, or platform change. Any new Orders or Analytics candidate must use at least 30 samples, perform zero order-data KV list/get work on warm/no-change hits, improve p95 latency by at least 40 percent over uncached fallback, remain fresh after every covered mutation/purge, and save enough CPU/backend reads to justify the additional billed inner Workers request.
