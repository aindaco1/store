...
- Inventory summaries separate the editable catalog/admin baseline from authoritative coordinator values: available, committed claims, and active reservations.
...
- Publishing uses consistent status typography, aligned phase markers, and an elapsed duration that stays together. Wide editors keep progress on one row; tablet and phone widths use explicit rows without splitting action labels. The shared status layout also covers bulk publishing. Editor field layouts respond to available container width and text size, and phone gutters leave more room for controls.
- Successful production publishes return the saved commit and deployment request. One operation-aware controller handles ordinary product edits, Active, Draft, Archived, and Sold out transitions, bulk status changes, and Save order. It shows an accessible **Saved → Media → Deploying → Deployed** indicator beside the editor’s publish action, the exact GitHub run state, operation-specific copy, and elapsed save-to-deploy time. The editor or arranged rows and prior product list remain in place until that run succeeds, then the dashboard refreshes from the newly deployed Worker catalog. A failed or unobservable run leaves the saved state explicit, preserves the prior list instead of presenting it as current, and links the GitHub run when available. Publishing automatically incorporates prepared media before deploying the storefront and Worker; a failed media job leaves the current storefront unchanged and enables retry through the same action.
...
After upload, the image field, media browser, and product preview use a shared in-memory copy immediately, including replacements at the same path. The canonical repository path remains the saved value. These temporary preview bytes are discarded on reload or sign-out. Publishing is disabled while an upload is in flight.
...
- Route misses share `CachedAdminStoreOrderIndex`, so immediate Orders and Analytics variants do not repeat a cold bounded index rebuild while KV propagation catches up. Inventory reads the SKU coordinator directly and does not scan orders.
...
- Order mutations purge dependent Orders and Analytics tags through the centralized dependency map. Inventory/coordinator mutations purge the inventory policy; purge failure does not fail the mutation, and short TTLs plus the manual refresh control bound stale exposure.
