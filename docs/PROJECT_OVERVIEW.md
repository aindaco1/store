...
Current release: `v1.1.2`. The published `v1.1.1` tag remains immutable as the previous release. The `v1.1.2` line advances the pinned shared-platform boundary and removes exact shared implementation copies without moving catalog, checkout, inventory, storage, or deployment authority out of Store.
