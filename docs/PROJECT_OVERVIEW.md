...
Current release: `v1.3.1`. The preceding `v1.3.0` release remains independently reversible. Ticket and RSVP products can opt into one deduplicated confirmed-purchaser thank-you email 24 hours after the configured event end. New event drafts default on. Enabling before the scheduled send includes earlier confirmed purchasers automatically; the setting locks at the cutoff and never triggers a historical send afterward. Super admins preview the branded English or Spanish message from unsaved Settings values and a selectable sample event without scanning or exposing recipients. Durable delivery, promotional suppression, and provider evidence stay separate from canonical order and fulfillment truth. Existing product publishing, status, checkout, RSVP registration, ticketing, email, and localization behavior remains intact. Store pins Platform v0.34.1 and the separate Jekyll Template v0.1.0; Store retains local build copies, configuration, content, production origins, credentials, deployment, and rollback authority.
