...
Current release: `v1.3.5`. The preceding `v1.3.4` release remains independently reversible. Newly added admins receive a 15-minute, one-time login link that opens the dashboard directly and rechecks their current authorization when redeemed. English and Spanish post-event mission editors share the same fixed height and icon-only link control. Their sandboxed preview shell lives under the private admin path so Cloudflare does not inject scripts into its opaque origin. Local Podman wrappers honor an explicit connection without taking over another task's VM. Durable delivery, promotional suppression, provider evidence, and canonical order and fulfillment truth remain separate. Store pins Platform v0.34.1 and the separate Jekyll Template v0.1.0; Store retains local build copies, configuration, content, production origins, credentials, deployment, and rollback authority.
