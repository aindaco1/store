...
Current release: `v1.1.7`. The preceding `v1.1.6` release remains independently reversible. Store pins Platform v0.17.0 and consumes its characterized signed-token, session-cookie, and same-origin mechanics without moving secret selection, notification-login policy, nonce/session records, roles/scopes, CSRF, routes, catalog, checkout, inventory, credentials, storage, or deployment authority out of Store.
