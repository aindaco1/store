...
Current release: `v1.1.3`. The published `v1.1.2` tag remains immutable as the previous release. The `v1.1.3` line advances the pinned shared-platform boundary through v0.15.0 and replaces characterized Worker HTTP/security, timezone, Stripe transport, and deterministic release-tool copies with thin Store adapters without moving catalog, checkout, inventory, storage, credentials, or deployment authority out of Store.
