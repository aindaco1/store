...
Current release: `v1.1.10`. The preceding `v1.1.9` release remains independently reversible. Store pins Platform v0.21.0 and consumes pure inventory state mechanics through an explicit catalog-refresh/preserve-claimed adapter without moving Durable Object/KV storage, checkout/order rules, deployment, or rollback authority out of Store.
