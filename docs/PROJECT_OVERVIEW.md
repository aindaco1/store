...
Current release: `v1.3.3`. The preceding `v1.3.2` release remains independently reversible. Ticket and RSVP products can opt into one deduplicated confirmed-purchaser thank-you email after the configured post-event delay. Super admins author distinct English and Spanish mission copy with constrained inline formatting and see a live, delivery-faithful preview without exposing recipients. Expired authenticated dashboard sessions return to the existing sign-in panel with clear localized guidance. Orders, fulfillment and attendee exports, and Analytics can be scoped to one canonical product; product Analytics attributes only matching persisted line-item revenue in mixed-product orders. Durable delivery, promotional suppression, provider evidence, and canonical order and fulfillment truth remain separate. Store pins Platform v0.34.1 and the separate Jekyll Template v0.1.0; Store retains local build copies, configuration, content, production origins, credentials, deployment, and rollback authority.
