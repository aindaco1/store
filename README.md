...
- Current release: `v1.3.4`. Analytics reports settled tip revenue from canonical order totals and proportionally attributes mixed-order tips when scoped to one product. Orders keeps its CSV and attendee export actions together on one responsive row. The post-event mission editors provide constrained inline formatting and a live delivery-faithful preview; expired authenticated dashboard sessions return to sign-in with clear guidance.
...
- The preceding `v1.3.3` release remains independently reversible.
...
Store `v1.3.4` pins Dust Wave Platform `v0.34.1` at exact commit
