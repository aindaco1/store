...
- Current release: `v1.2.3`. Active, draft, archived, and sold-out transitions, ordinary product edits, bulk status changes, and product reordering share one exact-commit deployment progress controller. Existing action labels, drag-and-drop, keyboard reordering, layouts, and status semantics remain unchanged; the dashboard keeps the saved edit visible and refreshes only after the matching GitHub deployment succeeds.
...
- The preceding `v1.2.2` release remains independently reversible.
