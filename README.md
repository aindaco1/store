...
- Product publishing keeps its save controls visible while editing, labels pending status transitions explicitly, and follows the exact GitHub deployment for the saved commit through requested, queued, running, and completed states. The dashboard measures save-to-deploy time, refreshes the catalog only after success, and links a failed run without misreporting the prior deployed status as current. Archived status remains fail-closed in static controls and Worker checkout validation. Platform Worker Core `0.12.1` uses Cloudflare-compatible fail-closed redirect handling so repository reads reach GitHub instead of failing locally with a 502. Product event addresses preserve multiple lines with responsive lookup controls; static previews remain isolated without blocked-script or disallowed-font console noise. Desktop field-help spacing and narrow-screen containment are covered by browser regressions.
