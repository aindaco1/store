...
- Current release: `v1.3.7`. Local product saves regenerate the canonical Worker catalog and wait for the saved snapshot before refreshing the dashboard. Service fulfillment is supported consistently, and the Paradiso event ticket is included as an operator-authored draft.
...
- The preceding `v1.3.6` release remains independently reversible.
...
- Source catalog: 51 `_products/*.md` records at this sweep, with 25 active, 25 archived, and 1 draft product across physical merch, event tickets, one digital product, and one free RSVP fixture.
...
Store `v1.3.7` pins Dust Wave Platform `v0.34.1` at exact commit
