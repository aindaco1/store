...
- Current release: `v1.3.2`. Ticket and RSVP products can send one branded thank-you and support email after the configured post-event delay. New event drafts default on; enabling an event before its scheduled send automatically includes all unique confirmed purchasers, including earlier orders, and the setting locks at the cutoff. Super admins edit distinct English and Spanish mission copy and see a live, delivery-faithful preview from unsaved values in Settings without exposing recipients or running a manual backfill.
...
- The preceding `v1.3.0` release remains independently reversible.
...
Store `v1.3.2` pins Dust Wave Platform `v0.34.1` at exact commit
