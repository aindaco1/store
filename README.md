...
- Current release: `v1.3.5`. Newly added admins receive a 15-minute, one-time link that opens the dashboard directly. The post-event mission editors use equal fixed heights and an icon-only link control, while the sandboxed preview inherits the private admin response policy so Cloudflare does not inject cross-origin scripts. Explicit Podman connections are honored without restarting another task's VM.
...
- The preceding `v1.3.4` release remains independently reversible.
...
Store `v1.3.5` pins Dust Wave Platform `v0.34.1` at exact commit
